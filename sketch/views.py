...
def note(request):
    ...
    notes = Note.objects.filter(note_user_id=request.user, notes_date__in=dayWeekday)
...
def add(request):
    ...
    note = Note(notes_title=request.POST['notes_title'], notes_description=request.POST['notes_description'],
                notes_time=request.POST['notes_time'], notes_date=request.POST['notes_date'], notes_priority=request.POST['notes_priority'])
    note.save()
